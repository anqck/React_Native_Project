...
const TransactionsStatistics = ({
  ...
  <View style={s.root}>
    ...
    <FlatList
      ...
      isSimpleEmptyText
      listEmptyText="Bạn không có giao dịch nào"
      ...
      ListHeaderComponent={
        <View>
          <View style={s.chartContainer}>
            <View style={s.pieContainer}>
              <VictoryPie
                ...
                colorScale={dataForChart.length ?  colorForChart : [colors.greyDarker]}
              ...
              <View style={s.pieTextContainer}>
                ...
                >
                    {Math.round(totalValue).toString().replace(/(.)(?=(\d{3})+$)/g,'$1,')+' ₫' }
                </Text>
          ...
          <Subtitle
            style={s.subtitle}
            withLittlePadding
            leftText="Thống kê theo danh mục"
          />
          <Separator />
        </View>
        }
...
TransactionsStatistics.navigationOptions = ({
  headerTitle: <HeaderTitle title="Thống kê" />,
});
